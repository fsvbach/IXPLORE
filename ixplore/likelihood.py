...
def heuristic_log_likelihood(
    answers: np.ndarray,
    likelihood: np.ndarray,
) -> np.ndarray:
    """Compute log-likelihood using the heuristic linear distance measure.
    ...
    """
    return np.sum(np.log(1 - np.abs(answers - likelihood) + 1e-600), axis=1)
