...
def test_compute_posterior_X(xplore: IXPLORE, answers: pd.Series) -> None:
    assert xplore.compute_posterior_X(answers).shape == (G,)


def test_embed_user(xplore: IXPLORE, answers: pd.Series) -> None:
    assert xplore.embed_user(answers).shape == (2,)


def test_impute_remaining_answers(xplore: IXPLORE, answers: pd.Series) -> None:
    out = xplore.impute_remaining_answers(answers)
